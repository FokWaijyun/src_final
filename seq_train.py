...
def train(args, data_info):
    ...
    if args.item_update_mode == 'plus_transform_with_features':  # 向量更新模式
    ...
    for epoch in range(args.n_epoch):
        ...
        print('epoch %d    train auc: %.4f  acc: %.4f    eval auc: %.4f  acc: %.4f '
              % (epoch, train_auc, train_acc, eval_auc, eval_acc))
...
def pipeline():
    ...
    if os.path.exists('kg_sequence_result.csv'):
        ...
        result = pd.DataFrame(columns=['time', 'density', 'diameter', 'node_num', 'edge_num', 'acc', 'auc'])
    # test
    if result.shape[0] == 0:
        time = "2021/12/01"
    else:
        time = result.loc[result.shape[0]]['time'].replace('-', '/')

    gap = datetime.datetime.strptime("2021/01/31", "%Y/%m/%d") - datetime.datetime.strptime(time, "%Y/%m/%d")
    for step in range(0, gap.days, 7):
        upper_time = datetime.datetime.strptime(time, "%Y/%m/%d") + datetime.timedelta(days=step)
        print(upper_time)
