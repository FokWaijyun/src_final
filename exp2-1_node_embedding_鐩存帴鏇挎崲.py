...
def replace_item_embedding(args, model, device):
    model.entity_emb
    print('开始载入商品特征')
    ...
    with torch.no_grad():
        for key, val in item_feat_dict.items():
            ...
            # 替换item向量
            model.entity_emb.weight[key] = torch.FloatTensor(val).to(device=device)
...
def train(args, data_info):
    ...
    replace_item_embedding(args, model, device)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--tensor_board_comment', type=str, default='特征嵌入-直接替换', help='可视化工具文件的后缀')
    parser.add_argument('--log_dir', type=str, default='./runs/特征嵌入-直接替换', help='可视化工具文件文件名')
